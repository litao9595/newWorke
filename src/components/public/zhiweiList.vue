<template>
    <div>
      <p class="text-1">职位类型选择</p>
      <p class="text-2">选择职位名称所对应的类型</p>
      <ul class="zhi-wei-list">
          <li @click="jobChoice(item.id)" v-for="(item,i) in zhiwei" v-if="item.pid==1">{{item.jobTypeName}}</li>
      ...
      <div class="poup-box" v-if="poupBox">
        <div class="poup-bg" @click="poupBox=false"></div>
          <div class="poup-content-box">
              <div class="left">
                <p @click="clicksecondLevel(i,item.id)" :class="secondLevelIndex==i?'active':''" v-for="(item,i) in firstlevelTree2">{{item.jobTypeName}}</p>
              </div>
              <div class="right">
                  <ul>
                    <li @click.stop="jobChoiceList(item.id,item.jobTypeName)" v-for="(item,i) in firstlevelTree3">{{item.jobTypeName}}</li>
                  </ul>
              </div>
          </div>
      </div>
...
export default {
  ...
  created(){
      ...
      this.$http({
        method:'post',
        url:config.getJobType+'?appId='+sessionStorage.appId+'&userType=0',
      }).then((res)=>{
  ...
  methods:{
    jobChoice(_id){
      ...
      if(this.firstlevelTree2.length){
        for(let k=0; k<this.zhiwei.length; k++){
          if(this.zhiwei[k].pid==this.firstlevelTree2[0].id){
            this.firstlevelTree3.push(this.zhiwei[k])
          }
        }
      }
    },
